...
ReadBytesResult ImageReader::readBytes(reflection::RemoteAddress Addr,
                                       uint64_t Size) {
  auto &Obj = *TheImage.getObjectFile();
  StringRef Contents = Obj.getData();
  if (Addr.getAddressData() + Size > Contents.size())
    return ReadBytesResult(nullptr, no_op_destructor);
  auto bytes = Contents.slice(Addr.getAddressData(), Size);
  return ReadBytesResult(bytes.data(), no_op_destructor);
}

bool ImageReader::readString(reflection::RemoteAddress Addr, std::string &Dest) {
  auto &Obj = *TheImage.getObjectFile();
  StringRef Contents = Obj.getData();
  if (Contents.empty())
    return false;
  ...
  for (unsigned i = 0; i < Contents.size(); ++i) {
    if (Contents[i] == 0) {
      ...
      Dest.append(Contents.begin(), Contents.begin() + i);
      return true;
    }
  }
  // No null terminator found in the binary.
  return false;
}

remote::RemoteAbsolutePointer ImageReader::resolvePointer(reflection::RemoteAddress Addr,
                                                          uint64_t pointerValue) {
  return remote::RemoteAbsolutePointer("", pointerValue);
}

remote::RemoteAbsolutePointer ImageReader::getDynamicSymbol(reflection::RemoteAddress Addr) {
  return nullptr;
}
