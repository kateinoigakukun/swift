...
class WasmStdlib(cmake_product.CMakeProduct):
    ...
    def build(self, host_target):
        ...
        # Explicitly set the CMake AR and RANLIB to force it to use llvm-ar/llvm-ranlib instead of
        # the system ar/ranlib, which usually don't support WebAssembly object files.
        self.cmake_options.define('CMAKE_AR:STRING', os.path.join(toolchain_path, 'bin', 'llvm-ar'))
        # llvm-ranlib is not installed in the final toolchain, so use one in build dir
        self.cmake_options.define('CMAKE_RANLIB:STRING', os.path.join(self._host_swift_build_dir(host_target), 'bin', 'llvm-ranlib'))
        self.cmake_options.define('SWIFT_NATIVE_CLANG_TOOLS_PATH:STRING', os.path.join(toolchain_path, 'bin'))
        self.cmake_options.define('SWIFT_NATIVE_SWIFT_TOOLS_PATH:STRING', os.path.join(toolchain_path, 'bin'))
        self.cmake_options.define('SWIFT_NATIVE_LLVM_TOOLS_PATH:STRING', os.path.join(toolchain_path, 'bin'))
        self.cmake_options.define('SWIFT_BUILD_RUNTIME_WITH_HOST_COMPILER:BOOL', 'FALSE')
        self.cmake_options.define('SWIFT_WASI_SYSROOT_PATH:STRING', self._wasi_sysroot_path(host_target))

        # It's ok to use the host LLVM build dir just for CMake functionalities
        llvm_cmake_dir = os.path.join(self._host_llvm_build_dir(host_target), 'lib', 'cmake', 'llvm')
        ...
        self.cmake_options.define('SWIFT_STDLIB_SINGLE_THREADED_CONCURRENCY:BOOL', 'TRUE')
        ...
        self.cmake_options.define('SWIFT_STDLIB_COMPACT_ABSOLUTE_FUNCTION_POINTER:BOOL', 'TRUE')
        self.cmake_options.define('SWIFT_ENABLE_EXPERIMENTAL_CONCURRENCY:BOOL', 'TRUE')
        self.cmake_options.define('SWIFT_ENABLE_EXPERIMENTAL_STRING_PROCESSING:BOOL', 'TRUE')
        ...
        lit_test_paths = [os.path.join(self.build_dir, 'test-wasi-wasm32', path) for path in lit_test_paths]
        self.cmake_options.define('SWIFT_LIT_TEST_PATHS:STRING', ';'.join(lit_test_paths))
        test_driver_options = [
            # TODO: To link properly, add compiler support of `__main_argc_argv` (rdar://116552600)
            '-Xlinker', '--allow-undefined',
            # compiler-rt is not installed in the final toolchain, so use one in build dir
            '-Xclang-linker', '-resource-dir=' + self._wasi_sysroot_path(host_target),
        ]
        # Leading space is needed to separate from other options
        self.cmake_options.define('SWIFT_DRIVER_TEST_OPTIONS:STRING', ' ' + ' '.join(test_driver_options))

        # Configure with WebAssembly target variant, and build with just-built toolchain
        self.build_with_cmake([], self._build_variant, [], prefer_just_built_toolchain=True)
    ...
    def test(self, host_target):
        ...
        env = { 'PATH': os.path.sep.join(bin_paths) }
    ...
    def _wasi_sysroot_path(self, host_target):
        build_root = os.path.dirname(self.build_dir)
